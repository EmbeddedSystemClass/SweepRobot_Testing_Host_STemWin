...
static enum COLLISION_CHAN gSwrbManualTestColliSionChan = COLLISION_CHAN_L;
...
{
    gSwrbTestTaskRunCnt = 0;
    gSwrbManualTestColliSionChan = COLLISION_CHAN_L;
...
static void SweepRobot_CollisionManualTestDataQuery(enum COLLISION_CHAN chan, int *dest_value)
{
    char *str;
    int i;
...
{
    SweepRobot_CollisionManualTestDataQuery(chan, value);
...
static void SweepRobot_CollisionManulTestDataProc(enum COLLISION_CHAN chan)
{
    char *str;

    if(!collision[chan].onValidFlag){
        if(!collision[chan].offValidFlag){
            SweepRobot_CollisionTestSteerMotorOffDataValidCmp(chan, &collision[chan].offValue, &collision[chan].offValidCnt, &collision[chan].offValidFlag);

            if(collision[chan].offValidFlag){
                switch(gSwrbManualTestColliSionChan){
            ...
            SweepRobot_CollisionTestSteerMotorOnDataValidCmp(chan, &collision[chan].onValue, &collision[chan].onValidCnt, &collision[chan].onValidFlag);
            ...
            if(collision[chan].onValidFlag){
                Listview_Set_Item_BkColor(hWin_SWRB_MANUAL, ID_MANUAL_LISTVIEW_MAIN, \
                                                        ...
                                                        gSwrbManualTestListviewDispDataCoord[SWRB_MANUAL_TEST_DATA_COLLISION_L_POS+chan][1],\
                                                        SWRB_MANUAL_TEST_OK_BK_COLOR);
                if(gSwrbManualTestColliSionChan == COLLISION_CHAN_L || gSwrbManualTestColliSionChan == COLLISION_CHAN_FL){
                    SweepRobot_CollisionCtrlLeftSteerMotorPosMove(STEER_MOTOR_IDLE_POS);
                }
                else{
                    SweepRobot_CollisionCtrlRightSteerMotorPosMove(STEER_MOTOR_IDLE_POS);
                ...
                gSwrbTestTaskRunCnt = 1;
                gSwrbManualTestColliSionChan++;

                if(gSwrbManualTestColliSionChan == COLLISION_CHAN_ALL){
                    gSwrbTestTaskRunCnt = 0;
                    gSwrbManualTestColliSionChan = COLLISION_CHAN_L;
...
{
    SweepRobot_CollisionManulTestDataProc(gSwrbManualTestColliSionChan);
}
    ...
    while(1){
        ...
        if(!Checkbox_Get_State(hWin_SWRB_PCBTEST, ID_PCBTEST_CHECKBOX_COLLISION)){
            ...
            if(gSwrbDialogSelectFlag == SWRB_DIALOG_SELECT_PCB){
                ...
                if(gSwrbTestTaskRunCnt > SWRB_COLLISION_TEST_TIME_OUT_CNT){
                    Listview_Set_Item_BkColor(hWin_SWRB_MANUAL, ID_MANUAL_LISTVIEW_MAIN,\
                                                           gSwrbManualTestListviewDispDataCoord[SWRB_MANUAL_TEST_DATA_COLLISION_L_POS+gSwrbManualTestColliSionChan][0],\
                                                           gSwrbManualTestListviewDispDataCoord[SWRB_MANUAL_TEST_DATA_COLLISION_L_POS+gSwrbManualTestColliSionChan][1],\
